...
# Apply SLIC superpixel segmentation to preserve spatial consistency
segments = slic(enhanced_image, n_segments=200, compactness=10, start_label=1)
superpixel_image = label2rgb(segments, enhanced_image, kind='avg')

# Reshape the image to be a list of pixels (data for K-Means)
Z = superpixel_image.reshape((-1, 3))
Z = np.float32(Z)
...
# Display the original image, superpixel image, segmented image, merged image, and the extracted foreground using OpenCV
cv2.imshow('Original Image', image)
cv2.imshow('Enhanced Image (CLAHE)', enhanced_image)
cv2.imshow('Superpixel Image (SLIC)', superpixel_image.astype(np.uint8))
