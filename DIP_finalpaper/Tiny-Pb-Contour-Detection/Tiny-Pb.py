...
def find_image_path(base_dir, image_name):
    ...
    for ext in extensions:
        pattern = os.path.join(base_dir, image_name + ext)
...
def PbLite(ImageName):
    ...
    ImagesDir = os.path.join(os.getcwd(), 'BSDS500', 'Images')
    AbsolutePathImage = find_image_path(ImagesDir, ImageName)

    # 打印调试信息
    print(f"当前工作目录: {os.getcwd()}")
    print(f"尝试读取的图像路径: {AbsolutePathImage}")

    if AbsolutePathImage is None:
        print(f"错误: 未找到图像 {ImageName} 在目录 {ImagesDir}")
        return  # 或者根据需要处理错误

    ColorImage = cv2.imread(AbsolutePathImage)

    if ColorImage is None:
        print(f"错误: 无法读取图像文件 - {AbsolutePathImage}")
        return  # 或者根据需要处理错误

    ColorImageShape = ColorImage.shape
    print(f"图像形状: {ColorImageShape}")
    ...
    DisplayFilterBank(DoGFilterBank, 4, 8, 'gray', os.path.join(ResultsPath, 'Filters'), 'DoG.png')
    ...
    DisplayFilterBank(LMFilterBank, 8, 12, 'gray', os.path.join(ResultsPath, 'Filters'), 'LM.png')
...
#   num_filters = NDImage.shape[2]
#   DN1Image = NDImage.reshape((-1, num_filters))
