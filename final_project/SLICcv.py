...
class SLIC:
    ...
    def _findLocalMinimum(self, center):
        ...
        for i in range(max(x_center - 1, 0), min(x_center + 2, self.width)):
            for j in range(max(y_center - 1, 0), min(y_center + 2, self.height)):
                if i + 1 < self.width and j + 1 < self.height:
                    c1 = self.labimg[j + 1, i]
                    c2 = self.labimg[j, i + 1]
                    c3 = self.labimg[j, i]
                    grad = abs(c1[0] - c3[0]) + abs(c2[0] - c3[0])
                    if grad < min_grad:
                        min_grad = grad
                        loc_min = [i, j]
        return loc_min
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("用法: python slic.py <图像路径> <超像素数量> <nc>")
        sys.exit(1)
    ...
    try:
        ...
        print("错误: <超像素数量> 和 <nc> 必须是整数。")
        sys.exit(1)

    img = cv2.imread(image_path)
    if img is None:
        print(f"错误: 无法加载路径为 {image_path} 的图像")
        sys.exit(1)
    ...
    slic.displayContours(color=(0, 0, 255))  # 例如：红色轮廓
